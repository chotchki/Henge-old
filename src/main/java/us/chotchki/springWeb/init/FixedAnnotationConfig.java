...
public class FixedAnnotationConfig extends AnnotationConfiguration {
	...
	public void createServletContainerInitializerAnnotationHandlers(
			WebAppContext context, List<ServletContainerInitializer> scis)
			throws Exception {
		...
		for (ServletContainerInitializer service : scis) {
			...
			if (annotation != null) {
				...
				if (classes != null) {
					...
					// If we haven't already done so, we need to register a
					// handler that will
					...
					for (Class c : classes) {
						// The value of one of the HandlesTypes classes is
						// actually an Annotation itself so
						// register a handler for it
						if (c.isAnnotation()) {
							if (LOG.isDebugEnabled())
								LOG.debug("Registering annotation handler for "
										+ c.getName());
					...
					if (LOG.isDebugEnabled())
						LOG.debug("No classes in HandlesTypes on initializer "
								+ service.getClass());
				}
			} else {
				initializer = new ContainerInitializer(service, null);
				if (LOG.isDebugEnabled())
					LOG.debug("No annotation on initializer "
							+ service.getClass());
			}
